...
- After achieving green CI, log a summary:
  ```bash
  mmlog --content "CI fixed for <repo>#<PR>: <root cause>" --tags rails,ci,fix
  ```
- If you give up and escalate to human (Attempt 4), log an error/learning:
  ```bash
  mmlog --content "CI still failing after escalation: <repo>#<PR>. Need human review." --tags ci,stuck
  ```
